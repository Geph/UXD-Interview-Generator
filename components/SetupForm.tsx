...
export const SetupForm: React.FC<SetupFormProps> = ({ onStart, showLogs, onSystemStatusChange }) => {
  ...
  const handleLaunch = async () => {
    ...
    setIsLaunching(true);
    const config: StudyConfig = { studyName, researchGoal, coreQuestions: parsedQuestions };
    
    addLog("Syncing study design to MySQL...");
    try {
      await databaseService.saveStudyConfig(config);
      addLog("Design persisted successfully. Launching...");
      onStart(config);
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Unknown sync error';
      addLog(`Sync Failure: ${msg}`);
      if (confirm(`Database Sync Failed: ${msg}\n\nStart interview session anyway? Data will only be logged to console.`)) {
        onStart(config);
      }
    } finally {
  ...
  const parsePaste = async () => {
    ...
    try {
      ...
      addLog(`Extracted ${extracted.length} question blocks.`);
      setMethod(null);
    } catch (err) {
    ...
    <div className="max-w-4xl mx-auto space-y-16 pb-32 animate-in fade-in slide-in-from-bottom-6 duration-1000">
      ...
      <section className="space-y-10">
        <div className="flex items-end justify-between px-2">
          <div className="space-y-2">
            ...
            <p className="text-slate-400 font-medium tracking-wide">Define your core milestones.</p>
          </div>
          <div className="flex gap-3">
            <button 
              onClick={() => setMethod(method === 'paste' ? null : 'paste')}
              className={`px-6 py-3 rounded-2xl text-xs font-bold uppercase tracking-widest border transition-all flex items-center gap-2 ${method === 'paste' ? 'bg-indigo-50 border-indigo-200 text-indigo-600' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}
            >
              {method === 'paste' ? 'Cancel Paste' : 'Paste Guide'}
            </button>
            <button 
        ...
        {method === 'paste' && (
          <div className="bg-indigo-50/50 rounded-[2rem] p-10 space-y-8 animate-in zoom-in-95 duration-300 shadow-xl border border-indigo-100">
            <div className="flex justify-between items-center">
              <div>
                <h3 className="text-slate-900 serif text-2xl font-semibold">Guide Synthesis</h3>
                <p className="text-slate-500 text-sm mt-1">Paste your text guide. Indented lines will be treated as follow-up probes.</p>
              </div>
            </div>
            <div 
              ref={editorRef}
              ...
              className="w-full min-h-[300px] bg-white border border-indigo-100 rounded-2xl p-8 text-slate-800 outline-none font-mono text-base focus:ring-4 focus:ring-indigo-500/10 transition-all"
              placeholder="Paste guide here..."
            ></div>
            <button 
              onClick={parsePaste}
              disabled={isExtracting}
              className="w-full py-6 bg-indigo-600 text-white font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-indigo-500 shadow-xl shadow-indigo-200/50 transition-all disabled:opacity-50"
            >
              {isExtracting ? (
        ...
        <div className="space-y-4">
          {parsedQuestions.map((q, idx) => (
            <div 
              ...
              className="group flex gap-6 items-start bg-white p-8 rounded-[2rem] border-2 border-slate-100 shadow-sm transition-all hover:border-indigo-100"
            >
              ...
              <button onClick={() => removeQuestion(q.id)} className="text-slate-200 hover:text-red-500 transition-colors">
      ...
      <section className="pt-10 flex flex-col items-center gap-6">
        <button 
          ...
          className={`px-16 py-8 rounded-[2.5rem] text-2xl font-black uppercase tracking-[0.2em] transition-all flex items-center gap-4 ${isFormValid && !isLaunching ? 'bg-slate-900 text-white shadow-2xl hover:scale-[1.02] active:scale-[0.98]' : 'bg-slate-100 text-slate-300 cursor-not-allowed'}`}
        >
          {isLaunching && <svg className="animate-spin h-6 w-6 text-white" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>}
          {isLaunching ? 'Persisting Design...' : 'Begin Interview Session'}
        </button>
      </section>
    </div>
  );
